...
class EmbeddingManager:
    ...
    def load_speaker(self, speaker_name: str) -> None:
        app.logger.info(f"Loading speaker: {speaker_name}")
    ...
    def get_embedding(self, speaker_name: str) -> torch.Tensor:
        ...
        if speaker_name not in self.embeddings:
            app.logger.info(f"Speaker {speaker_name} not loaded, loading now...")
            print(f"Speaker {speaker_name} not loaded, loading now...")
            self.load_speaker(speaker_name)
        
        embedding = self.embeddings[speaker_name]
        app.logger.debug(f"Retrieved embedding for {speaker_name} with shape: {embedding.shape}")
        return embedding
    ...
    def check_speaker_compatibility(self, speaker1: str, speaker2: str) -> Dict[str, any]:
        ...
        if speaker1 not in self.embeddings or speaker2 not in self.embeddings:
            raise ValueError(f"Um dos speakers não está carregado: {speaker1}, {speaker2}")
